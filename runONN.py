...
for epoch in range(n_epochs):
    ...
    for fold in tqdm(range(k)):
        ...
        train_acc, val_acc = run_epoch(model,train_dataloader=train_dataloader, val_dataloader=test_dataloader, criterion=criterion, optimizer=optimizer)
        ep_train.update(train_acc.avg)
        ep_val.update(val_acc.avg)
        print(f"fold {fold} [ train_acc:{train_acc.avg.item():.3f} | val_acc:{val_acc.avg.item():.3f}]")
    print(f"Epoch {epoch} [ train_acc:{ep_train.avg.item():.3f} | val_acc:{ep_val.avg.item():.3f}]")
